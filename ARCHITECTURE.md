...
| D70 | Ordenar la sangría es también un **botón**, y deja el cambio sin guardar | sangrar solo al guardar deja fuera todo lo que se escribió antes: nadie va a abrir y guardar dos mil unidades para verlas bien puestas, y quien abre una y la ve igual de desordenada concluye que la casilla no hace nada --que es exactamente lo que pasó--. No lo hace al abrir el fichero porque entonces cualquier unidad que se mire sale marcada como modificada, y «sin guardar» dejaría de significar «has escrito algo». Y no guarda en el mismo clic: reescribir el fichero de alguien y hacer el commit a la vez no deja sitio para mirar qué se ha reescrito, así que el cambio se queda a la vista con «Descartar» al lado |
| D71 | Partir líneas solo alrededor de los entornos que Didacta conoce | un salto de línea en LaTeX es un espacio, así que esto no puede hacerse a ciegas. Hay dos clases de corte: **sustituir** el espacio que ya hay --neutro siempre, porque un espacio, cuatro y un salto son el mismo token-- y **meter** uno donde no había nada, que sí añade un espacio. Lo segundo solo se hace con entornos de la lista de `classifyEnvironment`, que son de bloque y terminan en modo vertical, donde ese espacio se descarta. Con un entorno que no está en la lista no se toca ninguna línea, ni siquiera sustituyendo: `el conjunto \begin{math}A\end{math} es abierto` partido en tres líneas no se lee mejor, se lee partido. Sangrar sí se le hace, porque mover el margen izquierdo no puede cambiar nada. Comprobado a lo bruto: reindentado el repositorio de ejemplo entero, sus 8 PDFs salen **byte a byte idénticos** |
| D72 | Los números de línea se pintan en la misma capa que las barras de entorno | una caja de texto de Flutter no deja pintar entre sus líneas, así que las barras ya se pintaban detrás midiendo el texto aparte; una segunda capa para los números sería una tercera medida que descuadrar. Van en la **primera fila visual** de cada línea del fichero y no en cada renglón: una línea larga ocupa cuatro y sigue siendo una, y numerar los renglones sería numerar el ancho de la ventana. El ancho de la regleta se **mide** con el tipo de la caja en lugar de estimarse, porque de él depende dónde empieza el texto en las dos capas, y un fichero que pasa de 999 a 1000 líneas mueve todo lo demás con él |
| D73 | El beautify **junta** el párrafo antes de cortarlo | solo cortar deja igual de mal un párrafo que ya venía cortado a lo loco, y daría resultados distintos según cómo estuviera escrito antes; juntando primero, el resultado depende del texto y no de su historia, que es lo que hace que pasarlo dos veces dé lo mismo. No junta lo que lleva un comentario --metería la línea siguiente dentro del comentario-- ni lo que acaba en `\\` o en `%`, que son cortes del autor, ni una línea que es **solo** una orden: `\vspace{-2mm}` en mitad de una frase no se ve, y quien lo puso aparte lo puso aparte porque hace algo aparte. Corta solo en un espacio que esté fuera de las llaves y fuera de las matemáticas, y si no hay ninguno, la línea se queda larga: mejor larga que partida por el peor sitio |
| D74 | El espacio de alrededor de una marca de protección se copia del original, no del traductor | la traducción viaja en HTML --única forma de que los dos proveedores respeten las marcas-- y ahí el espacio pegado a una marca no es texto, es formato, así que lo mueven: `identificaremos $\mathbb Z$ con` volvía como `identificarem$\mathbb Z$ amb`. Es una propiedad de **la pieza** --si iba pegada o separada-- y no del sitio, así que copiarla sigue valiendo aunque la traducción cambie el orden de las palabras. Y «separada» es cualquier espacio en blanco, no un espacio concreto: un salto de línea separa igual y es el que tenía el fichero, así que poner un espacio delante de él dejaría un « \\n» ensuciando el diff de cada commit para siempre. Lo que sí es del traductor es el espacio entre palabras: reflotar un párrafo es traducir |
| D75 | La barra del editor se mide a doce anchos en un test | la misma cosa se rompió tres veces seguidas --entra un botón, se prueba en una ventana grande, se desborda en una mediana-- y el modo lado a lado hace paneles de un tercio de ventana, así que los anchos pequeños son tan reales como los grandes. Se prueba además **con el fichero tocado**, que es cuando salen «sin guardar» y «Descartar» y la barra va más llena: los tres desbordamientos fueron en ese estado. Lo que se comprueba no es el aspecto sino que no salte la excepción de renderizado y que el botón de guardar siga ahí, porque una barra que se desborda esconde su propio botón de guardar |
